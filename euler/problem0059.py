...
# subversion info:
# $HeadURL: https://skasi-lps.googlecode.com/svn/trunk/euler/problem0059.py $
# $Author: SkAsI.7 $
# $Id: problem0059.py 27 2011-01-24 15:42:09Z SkAsI.7 $
# $Revision: 27 $
...
def main():
  normal_range = range(32, 126)
  for key in permutations('abcdefghijklmnopqrstuvwxyz', 3):
    text = list()
    for c, k in zip(CIPHER, itertools.cycle(key)):
      text.append(c ^ ord(k))
      if text[-1] not in normal_range:
        break
    else:
      plain = ''.join(map(chr, text))
      if plain.count(' ') > 200: # Enough number of spaces
        return sum(text)
